...
export const rankingLevels = [
  ...
  },
  {
    keyIndex: 1,
    name: "Good Start",
    minScoreMultiplier: 0.125,
  },
  {
    keyIndex: 2,
    name: "Moving Up",
    minScoreMultiplier: 0.25,
  },
  {
    keyIndex: 3,
    name: "Good",
...
export default function Rankings(props: RankingsProps) {
  ...
  const currentRanking =
    rankingLevels[
      rankingLevels.filter(
        (i) => Math.floor(geniusScore * i.minScoreMultiplier) <= currentScore
      ).length - 1
    ];
  const nextRanking =
    rankingLevels[
                ...
                <div
                  ...
                  className={`${
                    i.minScoreMultiplier === currentRanking.minScoreMultiplier
                      ? "bg-yellow-300 rounded-full font-bold"
                      : i.minScoreMultiplier < currentRanking.minScoreMultiplier
                      ? "text-gray-400"
                      : ""
                  } px-4 py-2 flex flex-row justify-between items-center`}
                >
                  {i.minScoreMultiplier ===
                  currentRanking.minScoreMultiplier ? (
                    <div className="m-2 flex ">
                      <p>{currentScore}</p>
                    </div>
                  ) : (
                    <div
                      className={` w-2 h-2 m-2 mx-4 ${
                        i.minScoreMultiplier < currentRanking.minScoreMultiplier
                          ? "bg-yellow-300 rounded-full"
                          : i.minScoreMultiplier < 1
                  ...
                  <div>
                    ...
                    {i.minScoreMultiplier ===
                      ...
                      <div className="text-xs font-normal">
                        <p>
                          {Math.floor(
                            geniusScore * nextRanking.minScoreMultiplier
                          ) - currentScore}{" "}
                          points to next rank, {geniusScore - currentScore}{" "}
                          points to Genius
                        </p>
                      </div>
                    )}
